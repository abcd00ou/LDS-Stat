...
%
%
%  Function to price American Options using Edgeworth Expansion
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [Sn,Cn]=american_option_garch(b0,b1,b2,la,h1,mat,r,s0,k,dt);
%
...
%b=(.5^n)*(factorial(n)./(cumprod(j1).*fliplr(cumprod(j1)))); %Factorial trick!!!
b=(.5^n)*(factor(n)); %Factorial trick!!!
%Edgeworth expansion - Gram-Charlier
f=(1+(1/6)*mu3*(y.^3-3*y)+(1/24)*(mu4-3)*(y.^4-6*y.^2+3)).*b; %Gram-Charlier
%f=(1+(1/6)*mu3*(y.^3-3*y)+(1/24)*(mu4-3)*(y.^4-6*y.^2+3)+(1/72)*mu3^2*(y.^6-15*y.^4+45*y.^2-15)).*b; %Edgeworth expansion
